...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            return "error "," I have no users on my db. You will be able to interact with them if you reply to that person's message, or forward one of that user's messages."
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            return "error", "I don't seem to have interacted with this user before - feel free to forward a message from them to give me control! (Like a voodoo doll, I need a piece of being able to execute certain commands ...)"
        else:
...
def extract_unt_fedban(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found") and not str(user_id).isdigit():
            message.reply_text("I don't seem to have interacted with this user before - feel free to forward the message from "
                               "them to give me control! (Like a voodoo doll, I need a piece to make it"
